...
RDSResource.prototype.showInstanceDialog = function(){
  ...
  callToAddSlider('RDS' , instanceOptions.InstanceType, this.baseResource.element.attr('id'));
...
RDSResource.prototype.setOutputAttributes = function(configAttributes){
  var dialog = $('#'+this.baseResource.dialogId);
  dialog.find('#RDS_endpoint_ip').html("Endpoint IP : <code class='fnt_size'>"+configAttributes.get('endpoint_address')+"</code>");
  dialog.find('#RDS_endpoint_port').html("Endpoint Port : <code class='fnt_size'>"+configAttributes.get('endpoint_port')+"</code>");
}
